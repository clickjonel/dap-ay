<script setup>
    ...
    const roles = [
        ...
        'Barangay Voluntary Sanitary Inspector (BVSI)',
    ...
    const openEdit = (member) => {
        ...
        memberModal.value.mode    = 'edit'
        memberModal.value.member  = member
        memberModal.value.visible = true
    }
    ...
    const removeMember = (member) => {
        useForm({}).delete(`/teams/${props.team.id}/members/${member.id}`, {
            preserveScroll: true,
            onSuccess: () => {
                toast.add({ severity: 'success', summary: 'Member Removed', detail: 'Member has been removed successfully.', life: 2000 })
            },
        })
    }
        ...
        <form @submit.prevent="submit" class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-1.5">
                ...
                <Select
                    ...
                    show-clear
                    class="!text-sm w-full"
                    @change="(e) => { const user = props.users.find(u => u.id === e.value); if (user) form.name = user.name }"
                />
            </div>
