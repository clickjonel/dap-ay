<script setup>
    ...
    const changeStatus = (program) => {
        useForm({}).delete(`/program/${program.id}`, {
            onSuccess: () => {
                toast.add({ severity: 'success', summary: 'Status Updated', detail: 'The status of the program is changed successfully.', life: 3000 })
            },
            preserveScroll: true,
        })
    }
...
<template>
    <div class="h-full flex flex-col gap-4 lg:gap-5">
        ...
        <div class="flex items-start sm:items-center justify-between gap-3">
            <div>
                <h1 class="text-base lg:text-lg font-bold text-slate-800 leading-none">Programs</h1>
