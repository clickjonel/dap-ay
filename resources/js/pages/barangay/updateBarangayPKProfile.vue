...
const props = defineProps({
    ...
    pk_profile: { type: Object, default: () => null   },
})
...
const submit = () => {
    form.put(`/barangay/pk-profile/${props.pk_profile.id}`, {
        preserveScroll: true,
        onSuccess: () => {
            toast.add({
                severity: 'success',
        ...
        <div class="flex items-center gap-4 shrink-0">
            ...
            <div>
                <h1 class="text-lg font-bold text-slate-800 leading-none">
                    {{ props.pk_profile ? 'Edit PK Profile' : 'Add PK Profile' }}
                </h1>
                <p class="text-xs text-slate-400 mt-1">
                    {{ props.pk_profile.barangay.name }} —
                    {{ props.pk_profile.barangay.municipality?.name }},
                    {{ props.pk_profile.barangay.province?.name }}
                </p>
            </div>
            <span class="ml-auto text-[10px] font-mono font-semibold text-slate-400 bg-slate-100 px-2.5 py-1 rounded-full">
                #{{ props.pk_profile.barangay.id }}
            </span>
        </div>
                ...
                <form @submit.prevent="submit" class="px-5 py-5 flex flex-col gap-4">
                    ...
                    <div class="flex items-center justify-end gap-2 pt-1">
                        ...
                        >
                            ...
                            {{ props.pk_profile ? 'Update Profile' : 'Save Profile' }}
                        </button>
                    </div>
