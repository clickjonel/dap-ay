...
const form = useForm({
    longitude: props.geography.longitude ?? '',
    latitude:  props.geography.latitude  ?? '',
    is_gida:   props.geography.is_gida   ?? false,
})

const submit = () => {
    form.put(`/barangay/geography/${props.geography.id}`, {
        preserveScroll: true,
        onSuccess: () => {
            toast.add({
                severity: 'success',
                summary:  'Geography Updated',
                detail:   'Geography has been updated successfully.',
                life:     2000,
            })
...
<template>
    <div class="h-full flex flex-col gap-5 min-h-0">
        ...
        <div class="flex items-center gap-4 shrink-0">
            ...
            <div>
                <h1 class="text-lg font-bold text-slate-800 leading-none">Update Geography</h1>
