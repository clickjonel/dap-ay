...
const form = useForm({
    ...
    date_start:    props.activity.date_start    ? new Date(props.activity.date_start) : null,
    date_end:      props.activity.date_end      ? new Date(props.activity.date_end)   : null,
    total_clients: props.activity.total_clients ?? '',
    program_ids:   props.activity.programs?.map(p => p.id) ?? [],
    barangay_ids:  props.activity.barangays?.map(b => b.id) ?? [],
})
...
const selectedBarangays = ref(
    props.activity.barangays?.map(b => ({ id: b.id, name: b.name })) ?? []
)
...
const submit = () => {
    form
        ...
        .put(`/pk-activities/${props.activity.id}`, {
            preserveScroll: true,
            onSuccess: () => {
                toast.add({ severity: 'success', summary: 'Activity Updated', detail: 'Activity has been updated successfully.', life: 2000 })
...
<template>
    <div class="h-full flex flex-col gap-5 min-h-0">
        ...
        <div class="flex items-center gap-4 shrink-0">
            ...
            <div>
                <h1 class="text-lg font-bold text-slate-800 leading-none">Edit Activity</h1>
                <p class="text-xs text-slate-400 mt-1">Update the details of this activity.</p>
            </div>
